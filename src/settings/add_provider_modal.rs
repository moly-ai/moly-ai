...
impl WidgetMatchEvent for AddProviderModal {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions, scope: &mut Scope) {
        ...
        if self.button(cx, ids!(add_server_button)).clicked(actions) {
            ...
            let api_key = self.text_input(cx, ids!(api_key)).text().trim().to_string();
...
impl AddProviderModal {
    fn set_error_message(&mut self, cx: &mut Cx, message: &str) {
        ...
        self.label(cx, ids!(error_message)).set_text(cx, message);
    }

    fn clear_error_message(&mut self, cx: &mut Cx) {
        self.label(cx, ids!(error_message)).set_text(cx, "");
        self.view(cx, ids!(error_view)).set_visible(cx, false);
    }

    fn clear_form(&mut self, cx: &mut Cx) {
        self.text_input(cx, ids!(api_host)).set_text(cx, "");
        self.text_input(cx, ids!(api_key)).set_text(cx, "");
        self.clear_error_message(cx);
