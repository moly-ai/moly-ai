...
script_mod! {
    ...
    mod.widgets.AttachmentViewBase =
        #(AttachmentView::register_widget(vm))

    mod.widgets.AttachmentView =
        set_type_default() do mod.widgets.AttachmentViewBase {
        flow: Overlay
        icon_wrapper := View {
            flow: Down
...
impl AttachmentView {
    /// Sets the attachment to display, updating icon, title, and preview.
...
/// Returns whether this widget could generate a preview for the attachment.
